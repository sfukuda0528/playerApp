...
4. **両面評価**: 戻ってきた結果から次を記録する。
   ...
   - **指示側の計測**（判定規則は本節で一元定義、他箇所は本節を参照する）:
     ...
     - ステップ数（spawn_agent/wait_agent の usage メタに `tool_uses` があれば使う。無い環境では N/A）
     - 所要時間（spawn_agent/wait_agent の usage メタに `duration_ms` があれば使う。無い環境では N/A）
...
呼び出し側はレポートから自己申告部分を抽出し、`tool_uses` / `duration_ms` は spawn_agent/wait_agent の usage メタがある場合だけ取得する。無い環境では N/A として評価軸表を埋める。

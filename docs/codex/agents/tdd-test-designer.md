...
出力は `docs/codex/agent_output/tdd-test-designer` ディレクトリに保存。
...
`docs/codex/agent_memory/tdd-test-designer` にファイルベースの永続メモリシステムあり。このディレクトリは既存 — mkdir不要、apply_patchで直接書き込み可能。
